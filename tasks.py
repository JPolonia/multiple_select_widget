...
def coverage(c):
    ...
    """
    c.run("coverage run --source multiple_select_widget runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/multiple_select_widget.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ multiple_select_widget")
...
def lint(c):
    ...
    """
    c.run("flake8 multiple_select_widget tests")
...
def release(c, bumpsize=''):
    ...
    import multiple_select_widget
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=multiple_select_widget.__version__))
    c.run("git push --tags")
